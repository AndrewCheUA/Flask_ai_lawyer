...
from database.sql_db.models import User
...
class RegistrationForm(FlaskForm):
    ...
    def validate_username(self, username):
        user = session_db.query(User).filter_by(username=username.data).first()
        if user:
            raise ValidationError(f'User with username: {username.data}, already exists.')

    def validate_email(self, email):
        user = session_db.query(User).filter_by(email=email.data).first()
        if user:
...
class PostForm(FlaskForm):
    ...
    content = TextAreaField('Content', validators=[DataRequired(), Length(min=2, max=900)])
